<?php
/**
 * WP Bootstrap Navwalker Vertical
 *
...
if ( ! class_exists( 'WP_Bootstrap_Navwalker_Vertical' ) ) :
	/**
	 * WP_Bootstrap_Navwalker class.
	 */
	class WP_Bootstrap_Navwalker_Vertical extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			$id = '';
			if ( $this->_lastID ) {
				$id = ' id="menu-item-collapse-' . $this->_lastID . '"';
			}

			/*
			 * The `.collapse-menu` container needs to have a labelledby
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			// Add .collapse or .active classes where they are needed.
			...
			foreach ($classes as $key => $class) {
				if ( substr($class, 0, 3) == "btn" ) {
					$anchor_classes[] = $class;
					$has_button = true;
				}
			}
			if ( $has_button ) { $classes[] = "has-btn"; }
			$classes = array_diff($classes, $anchor_classes);
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				$atts['class']          = 'nav-link';
				$atts['data-bs-target'] = '#menu-item-collapse-' . $item->ID;
				...
				// For items in collapse use .collapse-item as well as .nav-link.
				if ( $depth > 0 ) {
					$atts['class'] = 'collapse-item nav-link';
				} else {
					$atts['class'] = 'nav-link';
				}
			}
		 ...
		 *
		 * Supported linkmods: .disabled, .collapse-header, .collapse-divider, .sr-only
		...
		private function separate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, $depth ) {
			...
			foreach ( $classes as $key => $class ) {
				...
				if ( preg_match( '/^disabled|^sr-only/i', $class ) ) {
					...
					unset( $classes[ $key ] );
				} elseif ( preg_match( '/^collapse-header|^collapse-divider|^collapse-item-text/i', $class ) && $depth > 0 ) {
					/*
					 * Test for .collapse-header or .collapse-divider and a
					 * depth greater than 0 - IE inside a collapse.
					 */
					$linkmod_classes[] = $class;
					unset( $classes[ $key ] );
			...
			if ( ! empty( $linkmod_classes ) ) {
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						if ( 'collapse-header' === $link_class ) {
							$linkmod_type = 'collapse-header';
						} elseif ( 'collapse-divider' === $link_class ) {
							$linkmod_type = 'collapse-divider';
						} elseif ( 'collapse-item-text' === $link_class ) {
							$linkmod_type = 'collapse-item-text';
						}
					}
				}
			}
				...
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						if ( 'disabled' === $link_class ) {
							...
							$atts['href'] = '#';
							unset( $atts['target'] );
						} elseif ( 'collapse-header' === $link_class || 'collapse-divider' === $link_class || 'collapse-item-text' === $link_class ) {
		...
		private function linkmod_element_open( $linkmod_type, $attributes = '' ) {
			$output = '';
			if ( 'collapse-item-text' === $linkmod_type ) {
				$output .= '<span class="collapse-item-text"' . $attributes . '>';
			} elseif ( 'collapse-header' === $linkmod_type ) {
				/*
				 * For a header use a span with the .h6 class instead of a real
				 * header tag so that it doesn't confuse screen readers.
				 */
				$output .= '<span class="collapse-header h6"' . $attributes . '>';
			} elseif ( 'collapse-divider' === $linkmod_type ) {
				// This is a divider.
				$output .= '<div class="collapse-divider"' . $attributes . '>';
			}
			return $output;
		}
		...
		private function linkmod_element_close( $linkmod_type ) {
			$output = '';
			if ( 'collapse-header' === $linkmod_type || 'collapse-item-text' === $linkmod_type ) {
				/*
				 * For a header use a span with the .h6 class instead of a real
				 * header tag so that it doesn't confuse screen readers.
				 */
